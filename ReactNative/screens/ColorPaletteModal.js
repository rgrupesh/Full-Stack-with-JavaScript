...
import { Text, View, TextInput, StyleSheet, Alert , Switch , FlatList} from "react-native";
...
const ColorPaletteModal = ({ navigation}) => {
    ...
    const handleSubmit = useCallback(() => {
        if (!name){
            ...
            Alert.alert("Please add atleast 3 items ");
        }    
         else {
    ...
    const handleValueChange = useCallback((value, color) => {
        if (value === true){
            ...
            setSelectedColor(colors => colors.filter(selectedColor => color.colorName !== selectedColor.colorName));
        }
    ...
    return(
        ...
        <FlatList 
            ...
            renderItem={({ item }) => (
                ...
                <Switch value={!!selectedColor.find(color => color.colorName === item.colorName)} 
